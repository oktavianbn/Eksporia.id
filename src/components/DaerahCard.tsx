...
const DaerahCard: React.FC<DaerahCardProps> = ({ data, onClose }) => {
  ...
  return (
    <div
      ...
      className={`fixed h-screen inset-0 z-50 flex items-center justify-center p-4 transition-all duration-300 bg-transparent"`}
    >
      {/* Modal */}
      <div
        className={`p-4 relative w-full max-w-4xl rounded-3xl shadow-3xl overflow-hidden transition-all duration-300 transform hover:scale-[1.02] ${isDarkMode ? "bg-gray-800/90 border border-gray-700" : "bg-white/90 border border-gray-200"
          }`}
      >
        ...
        <div className="flex flex-col lg:flex-row">
          ...
          <div className="lg:w-1/2 px-4 flex flex-col">
            ...
            <div className="mb-4">
              <h2
                className={`text-2xl sm:text-3xl font-bold mb-3 ${isDarkMode ? "text-white" : "text-gray-900"
                  }`}
              >
                {data.nama}
              </h2>
              ...
              <div className="flex gap-3 mb-4">
                ...
                <button
                  onClick={handleShare}
                  className={`flex items-center gap-2 px-4 py-2 rounded-full font-medium transition-all ${isDarkMode
            ...
            <div className="mb-4">
              ...
              <div className="flex flex-wrap gap-2">
                {data.kategori.map((k, i) => (
                  <span
                    key={i}
                    className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium transition-all hover:scale-105 ${isDarkMode
            ...
            <div className="mb-4 flex-1">
              ...
              >
                {visibleKomoditas.map((item, i) => (
                  <div
                    key={i}
                    className={`flex items-center gap-3 p-3 rounded-xl transition-all md:hover:scale-105 ${isDarkMode ? "bg-gray-700 hover:bg-gray-600" : "bg-gray-50 hover:bg-gray-100"
                      }`}
                  >
                    <div
              ...
              {data.komoditas.length > 3 && (
                <button
                  ...
                  className={`mt-2 px-3 py-1 text-sm rounded-md font-medium ${isDarkMode
                    ? "bg-gray-700 text-gray-300 hover:bg-gray-600"
                    : "bg-gray-100 text-gray-700 hover:bg-gray-200"
                    }`}
                >
            ...
            <div className="flex justify-end">
              <button
                onClick={onClose}
                className={`px-6 py-2 sm:px-8 sm:py-3 rounded-xl font-semibold transition-all hover:scale-105 active:scale-95 ${isDarkMode
        ...
        {/* Decorative elements */}
        <div
          className={`absolute top-0 left-0 w-32 h-32 rounded-full opacity-10 -translate-x-16 -translate-y-16 ${isDarkMode ? "bg-blue-400" : "bg-blue-500"
            }`}
        ></div>
        <div
          className={`absolute bottom-0 right-0 w-24 h-24 rounded-full opacity-10 translate-x-12 translate-y-12 ${isDarkMode ? "bg-purple-400" : "bg-purple-500"
            }`}
        ></div>
      </div>
  );
